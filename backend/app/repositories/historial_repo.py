...
async def get_for_context(db: AsyncSession, context_type: str, context_id: str) -> list[Historial]:
    if context_type == "tienda":
        stmt = (
            select(Historial)
            .outerjoin(Documento, (Historial.entidad == "documento") & (Historial.entidad_id == Documento.id))
            .outerjoin(Tramite, (Historial.entidad == "tramite") & (Historial.entidad_id == Tramite.id))
            .outerjoin(Alerta, (Historial.entidad == "alerta") & (Historial.entidad_id == Alerta.id))
            .where(
                or_(
                    (Historial.entidad == "tienda") & (Historial.entidad_id == context_id),
        ...
        stmt = (
            select(Historial)
            .outerjoin(Documento, (Historial.entidad == "documento") & (Historial.entidad_id == Documento.id))
            .outerjoin(documento_tramites, Documento.id == documento_tramites.c.documento_id)
            .where(
                or_(
                    (Historial.entidad == "tramite") & (Historial.entidad_id == context_id),
