...
import { Activity, TrendingUp, TrendingDown, Minus, Clock, FileText, CheckCircle, AlertTriangle, XCircle, Store, Plus } from 'lucide-react';
...
export function OperatorPerformance({ userId }: OperatorPerformanceProps) {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => api.get<OperatorPerformanceData>(`/api/usuarios/${userId}/performance?range=${range}`),
  });
  ...
  const renderMetricCard = (
    ...
    const trendText = metric.trend === 'up' 
      ? `+${Math.abs(metric.value - metric.previous_value).toFixed(isNegativeMetric ? 1 : 0).replace(/\.0$/, '')}` 
      : metric.trend === 'down' 
        ? `-${Math.abs(metric.previous_value - metric.value).toFixed(isNegativeMetric ? 1 : 0).replace(/\.0$/, '')}` 
        : 'Sin cambio';
    ...
    return (
      <div className="bg-surface rounded-xl border border-border p-6 flex flex-col hover:border-border-strong transition-colors shadow-sm">
        <div className="flex items-center gap-3 text-text-secondary mb-4">
          <div className="p-2.5 rounded-lg bg-neutral-light text-text-primary">
            {icon}
          </div>
        ...
        <div className={`flex items-center gap-1.5 mt-3 text-sm font-semibold ${trendColor}`}>
          ...
          <span>{trendText}</span>
          <span className="text-text-muted text-xs font-normal ml-1 hidden sm:inline">vs periodo ant.</span>
        </div>
      </div>
    );
  };

  const groupedTimeline = data?.timeline.slice(0, visibleCount).reduce((acc, item) => {
    const dateStr = new Date(item.fecha).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' });
        ...
        <div className="flex items-center bg-surface border border-border rounded-lg p-1">
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
              range === '30' ? 'bg-surface-card shadow-sm text-text-primary' : 'text-text-secondary hover:text-text-primary'
            }`}
          >
            Últimos 30 días
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
              range === 'month' ? 'bg-surface-card shadow-sm text-text-primary' : 'text-text-secondary hover:text-text-primary'
            }`}
          >
            Mes en curso
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
              range === '90' ? 'bg-surface-card shadow-sm text-text-primary' : 'text-text-secondary hover:text-text-primary'
            }`}
          >
            Últimos 90 días
          </button>
      ...
      {isError ? (
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {renderMetricCard('Documentos cargados', data?.metrics.documentos_cargados, <FileText className="w-4 h-4" />)}
            {renderMetricCard('Trámites resueltos', data?.metrics.tramites_resueltos, <CheckCircle className="w-4 h-4" />)}
            {renderMetricCard('Alertas atendidas', data?.metrics.alertas_atendidas, <AlertTriangle className="w-4 h-4" />)}
            {renderMetricCard('Tiempo promedio', data?.metrics.tiempo_promedio_resolucion, <Clock className="w-4 h-4" />, true, 'días')}
            {renderMetricCard('Trámites vencidos', data?.metrics.tramites_vencidos_responsabilidad, <XCircle className="w-4 h-4" />, true)}
          </div>

          <div>
            <h3 className="text-sm font-bold text-text-primary mb-4 uppercase tracking-wider">Actividad Reciente</h3>
            {isLoading ? (
                ...
                {Object.entries(groupedTimeline).map(([dateStr, items]) => (
                  <div key={dateStr} className="space-y-4">
                    <div className="flex items-center gap-4">
                      <h4 className="text-xs font-semibold text-text-secondary uppercase tracking-wider">{dateStr}</h4>
                        ...
                        <div key={item.id} className="relative">
                          ...
                          <div className="pl-2">
                            ...
                            <div className="flex items-center gap-2 mt-1.5 text-sm text-text-secondary">
                              ...
                              <Link to={`/tiendas/${item.tienda_id}`} className="hover:text-accent hover:underline font-medium">
                                {item.tienda_nombre}
                              </Link>
                              {item.tramite_nombre && (
                                <>
                                  <span className="text-border">•</span>
                                  <Link to={`/tiendas/${item.tienda_id}/tramites/${item.tramite_id}`} className="hover:text-accent hover:underline truncate max-w-[250px]">
                                    {item.tramite_nombre}
                                  </Link>
                                </>
                              )}
