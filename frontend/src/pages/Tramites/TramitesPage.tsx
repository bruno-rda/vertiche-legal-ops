...
export function TramitesPage() {
  ...
  const quickFilterValue = soloVencidos
    ? 'solo_vencidos'
    : porVencerDias === '30'
      ? 'por_vencer_30'
      : porVencerDias === '60'
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-text-secondary mt-1">
          Vista global de todos los trámites de la red
        </p>
      </div>
      <div className="flex flex-wrap items-center gap-3">
        <SearchInput
          ...
          placeholder="Buscar trámite o tienda..."
        ...
        <div className="relative">
          <select
            value={estadoGeo}
            onChange={(e) => up('estado_geografico', e.target.value)}
            className="appearance-none pl-3 pr-8 py-2 text-sm bg-surface-card border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 cursor-pointer"
          >
        ...
        {hasFilters && (
          <button
            onClick={clear}
            className="text-sm text-text-secondary hover:text-text-primary underline ml-2"
          >
            Limpiar filtros
          </button>
        )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    {['Tienda', 'Trámite', 'Tipo', 'Estado', 'Vencimiento', 'Días', 'Docs'].map(
                      (h) => (
                        <th
                          key={h}
                ...
                <tbody>
                  {data.data.map((t) => {
                    ...
                    return (
                      <tr
                        key={t.id}
                        onClick={() => navigate(`/tiendas/${t.tienda_id}/tramites/${t.id}`)}
                      ...
                      >
                        ...
                        <td
                          className={`px-4 py-3.5 text-sm font-semibold ${days < 0 ? 'text-danger' : days <= 15 ? 'text-warning' : 'text-text-secondary'}`}
                        >
