  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none pl-3 pr-8 py-2 text-sm bg-surface-card border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 cursor-pointer"
          >
        ...
        <div className="relative">
          <select
            value={ec}
            onChange={(e) => up('estado_cumplimiento', e.target.value)}
            className="appearance-none pl-3 pr-8 py-2 text-sm bg-surface-card border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 cursor-pointer"
          >
        ...
        {user?.rol === 'ADMIN' && usuarios && (
          <div className="relative">
            <select
              value={operadorId}
              onChange={(e) => up('operador_id', e.target.value)}
              className="appearance-none pl-3 pr-8 py-2 text-sm bg-surface-card border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 cursor-pointer"
            >
      ...
      <div className="bg-surface-card rounded-xl border border-border overflow-hidden">
        {isLoading ? (
          <TableSkeleton rows={8} cols={user?.rol === 'ADMIN' ? 6 : 5} />
        ) : isError ? (
          <EmptyState
            variant="error"
              ...
              <table className="w-full">
                ...
                <tbody>
                  {data.data.map((t) => (
                    <tr
                      key={t.id}
                      onClick={() => navigate(`/tiendas/${t.id}`)}
                      className="border-b border-border last:border-b-0 hover:bg-surface/60 cursor-pointer transition-colors"
                    >
                      ...
                      {user?.rol === 'ADMIN' && (
                        <td className="px-4 py-3.5 text-sm text-text-secondary">
                          {usuarios ? (
                            usuarios.find(
                              (u) => u.rol === 'OPERATOR' && u.tiendas_asignadas?.includes(t.id),
                            )?.nombre || <span className="text-text-muted italic">Sin asignar</span>
                          ) : (
                      ...
                      <td className="px-4 py-3.5 min-w-[160px]">
                        <ProgressBar value={t.cumplimiento} showLabel size="sm" />
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1.5">
                          {t.tramites_vencidos > 0 && (
                            <Badge variant="vencido" size="sm">
                              {t.tramites_vencidos}
                            </Badge>
                          )}
                          {t.tramites_por_vencer > 0 && (
                            <Badge variant="por_vencer" size="sm">
                              {t.tramites_por_vencer}
                            </Badge>
                          )}
                          {(t.tramites_vencidos > 0 || t.tramites_por_vencer > 0) && (
